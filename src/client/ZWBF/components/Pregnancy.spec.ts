	...
	describe("Event System", () => {
		describe("Timer Events", () => {
			...
			describe("Every minute update", () =>  {
				...
				])("Pregnancy progress is $progress and labor is $isInLabor", ({ current, isInLabor }) => {
					beforeEach(() =>  {
						jest.spyOn(Player.prototype, 'pregnancy', 'get')
						.mockReturnValue(mock<PregnancyData>({ current, isInLabor }));
					});
			...
			describe("Every Hour update", () => {
				...
				("should call appropriate effects when pregnancy progress is $progres", ({ progress, expected }) => {
					jest.spyOn(Player.prototype, "pregnancy", "get")
					.mockReturnValue(mock<PregnancyData>({ progress }));
			...
			describe("Every Day update", () => {
				...
				it.each([
					{ progress: 0.01, expected: () => expect(setFoodSicknessLevel).not.toHaveBeenCalled() },
					{ progress: 0.34, expected: () => expect(setFoodSicknessLevel).not.toHaveBeenCalled() },
					{ progress: 0.06, expected: () => expect(setFoodSicknessLevel).toHaveBeenCalled() },
				])("should call appropriate effects when pregnancy progress is $progress", ({ progress, expected }) => {
					jest.spyOn(Player.prototype, 'pregnancy', 'get')
					.mockReturnValue(mock<PregnancyData>({ progress }));
	...
	describe("Debug", () => {
		describe("Pregnancy data not defined", () => {
			...
			it.each<{
				...
				{ method: "advance", data: null, args: 10, expected: () => expect(spyPregnancySet).not.toHaveBeenCalled() },
				{ method: "advanceToLabor", data: null, expected: () => expect(spyPregnancySet).not.toHaveBeenCalled() },
				{ method: "advance", data: mock<PregnancyData>({ current: undefined }), args: 10, expected: () => expect(spyPregnancySet).toHaveBeenCalled() },
				{ method: "advanceToLabor", data: mock<PregnancyData>({ current: undefined }), expected: () => expect(spyPregnancySet).toHaveBeenCalled() },
			])("Method $method should have expected result when data is $data", ({ method, data, args, expected }) => {
