...
export class ZWBFUI {
	...
	constructor(props: ZWBFUIProps) {
		...
		Events.onCreatePlayer.addListener((undefined, player) => this.onCreatePlayer(player));
	...
	onCreateUI() {
		...
		// === Womb ===
		this.UI.addText(this.UIElements.womb.title, this.label("IGUI_ZWBF_UI_Womb_title"), undefined, "Center");
		this.UI.nextLine();
		this.UI.addImage(this.UIElements.womb.image, "media/ui/womb/normal/womb_normal_0.png");
		this.UI.nextLine()
		this.UI.addText(this.UIElements.womb.sperm.current.title, this.label("IGUI_ZWBF_UI_Current"), undefined, "Center");
		this.UI.addText(this.UIElements.womb.sperm.current.amount, "0 ml", undefined, "Center")
		this.UI.nextLine();
		this.UI.addText(this.UIElements.womb.sperm.total.title, this.label("IGUI_ZWBF_UI_Total"), undefined, "Center");
		this.UI.addText(this.UIElements.womb.sperm.total.amount, "0 ml", undefined, "Center");
		this.UI.nextLine();
		this.UI.addText(this.UIElements.womb.cycle.title, getText("IGUI_ZWBF_UI_Cycle"), undefined, "Center");
		this.UI.nextLine();
		this.UI.addText(this.UIElements.womb.cycle.phase.title, this.label("IGUI_ZWBF_UI_Phase"), undefined, "Center");
		this.UI.addText(this.UIElements.womb.cycle.phase.value, "", undefined, "Center");
		this.UI.nextLine();

		if(!this.player.HasTrait(ZWBFTraitsEnum.INFERTILE)) {
			this.UI.addText(this.UIElements.womb.fertility.title, this.label("IGUI_ZWBF_UI_Fertility"), undefined, "Center");
		...
		// Lactation UI
		this.UI.addImage(this.UIElements.lactation.image, "media/ui/lactation/boobs/color-0/normal_empty.png");
		this.UI.nextLine();
		this.UI.addText(this.UIElements.lactation.title, this.label("IGUI_ZWBF_UI_Milk_Amount"), undefined, "Center");
		this.UI.addImage(this.UIElements.lactation.level, "media/ui/lactation/level/milk_level_0.png");
	...
	onUpdateUI() {
		...
		if (!this.player.HasTrait("Infertile")) {
			this.UI[this.UIElements.womb.fertility.title].setText(getText(`IGUI_ZWBF_UI_${pregnancy ? "Pregnancy" : "Fertility"}`));
			this.UI[this.UIElements.womb.fertility.bar].setValue(pregnancy ? pregnancy.progress : fertility);
			this.UI[this.UIElements.womb.fertility.value].setText(`${math.floor(fertility * 100)}%`);
		}
	}
}