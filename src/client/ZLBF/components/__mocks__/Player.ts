...
export class Player<T = unknown> {
	...
	hasTrait(trait: ZLBFTraitsEnum) {
		return Player.hasTrait(this.player, trait);
	}

	addTrait(trait: ZLBFTraitsEnum) {
	...
	removeTrait(trait: ZLBFTraitsEnum) {
		const player = this.player;
		if (!player) return;
	...
	static hasTrait(player: IsoPlayer | undefined, trait: ZLBFTraitsEnum) {
